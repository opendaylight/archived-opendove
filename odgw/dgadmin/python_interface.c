...
PyObject *process_cli_data(PyObject *self, PyObject *args)
{
	...
	else
	{
		...
		ret = dove_gateway_cli_callback(cli_data);
	}
...
void * cli_thread(void *params)
{
	...
	if (status != DOVE_STATUS_OK)
	{
		log_debug(PythonLibLogLevel, "Failure %s", DOVEStatusToString(status));
		return NULL;
	}
...
dove_status python_lib_embed_cli_thread_start(int fExitOnCtrlC)
{
	...
	if (status != DOVE_STATUS_OK)
	{
		log_debug(PythonLibLogLevel, "Failure %s",
	...
	log_debug(PythonLibLogLevel, "Exit :%s",
	          DOVEStatusToString(status));
	return status;
