...
public class OpenStackIPAllocationPool {
    ...
    public OpenStackIPAllocationPool() { }

    public OpenStackIPAllocationPool(String lowAddress, String highAddress) {
     ...
     */
    public List<OpenStackIPAllocationPool> splitPool(String ipAddress) {
        List<OpenStackIPAllocationPool> ans = new ArrayList<OpenStackIPAllocationPool>();
        long gIP = OpenStackIPAllocationPool.convert(ipAddress);
        long sIP = OpenStackIPAllocationPool.convert(poolStart);
        long eIP = OpenStackIPAllocationPool.convert(poolEnd);
        long i;
        OpenStackIPAllocationPool p = new OpenStackIPAllocationPool();
        ...
        for (i=sIP; i<=eIP; i++) {
            ...
            if (i == eIP) {
                if (i != gIP) {
                    ...
                    p.setPoolEnd(OpenStackIPAllocationPool.longtoIP(i-1));
                }
                ans.add(p);
            }
            if (i != sIP && i != eIP) {
                if (i != gIP) {
                    if (!poolStarted) {
                        p.setPoolStart(OpenStackIPAllocationPool.longtoIP(i));
                        poolStarted = true;
                    }
                } else {
                    p.setPoolEnd(OpenStackIPAllocationPool.longtoIP(i-1));
                    poolStarted = false;
                    ans.add(p);
                    p = new OpenStackIPAllocationPool();
                }
            }
        }
        return ans;
